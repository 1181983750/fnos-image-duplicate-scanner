...
STATIC_DIR = Path(os.getenv("STATIC_DIR", str(Path(__file__).resolve().parent / "static")))
SCAN_ROOTS = [
    Path(root).resolve()
...
def load_image(path: Path) -> Image.Image:
    try:
        ...
        raise RuntimeError(f"无法识别图片格式: {path}") from exc


def save_jxl(source_path: Path, target_path: Path) -> None:
    ...
    with Image.open(source_path) as image:
        if ext in {".jpg", ".jpeg"}:
            image.save(target_path, format="JXL", lossless=True)
        else:
            normalized = ImageOps.exif_transpose(image)
            normalized.save(target_path, format="JXL", quality=90)
...
def probe_image_size(path: Path) -> tuple[int, int, float]:
    open_started = time.perf_counter()
    with Image.open(path) as opened:
        image = ImageOps.exif_transpose(opened)
        width, height = image.size
    return width, height, time.perf_counter() - open_started


def image_for_web(image: Image.Image) -> Image.Image:
...
def save_webp_image(path: Path, max_edge: int, quality: int) -> Path:
    with Image.open(path) as opened:
        image = ImageOps.exif_transpose(opened)
        image.thumbnail((max_edge, max_edge), Image.Resampling.LANCZOS)
        image = image_for_web(image)
...
def scan_image(
    ...
    try:
        ...
        if job is not None and sha_already_seen(job, record.sha256):
            ...
            try:
                record.width, record.height, record.decode_seconds = probe_image_size(path)
            except Exception:
                pass
            return record
...
async def thumbnail(background_tasks: BackgroundTasks, path: str = Query(...)):
    ...
    try:
        tmp_path = save_webp_image(image_path, max_edge=360, quality=80)
...
async def preview(background_tasks: BackgroundTasks, path: str = Query(...)):
    ...
    try:
        tmp_path = save_webp_image(image_path, max_edge=1600, quality=86)
        background_tasks.add_task(tmp_path.unlink, missing_ok=True)
